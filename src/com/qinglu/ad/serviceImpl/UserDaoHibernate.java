...
public class UserDaoHibernate extends HibernateDaoSupport implements UserDao {
	...
	public List<User> getUsersByIndex(int index) {
		long start = index*20;
		long end = (index+1)*20;
		List users = getHibernateTemplate().find("from User u where u.id > ? and u.id <= ?",
				new Object[]{start,end});
		return users;
	}

	public long getUsersNum() {		
		Query query = this.getSession().createQuery("select count(*) from User");
